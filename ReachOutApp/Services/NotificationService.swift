...
protocol NotificationServiceProtocol {
    ...
    func scheduleNotifications(for person: TrackedPerson, birthdayEnabled: Bool)
    func removeNotifications(for person: TrackedPerson)
    func refreshBirthdayNotifications(for people: [TrackedPerson], enabled: Bool)
}
...
final class NotificationService: NotificationServiceProtocol {
    ...
    func refreshBirthdayNotifications(for people: [TrackedPerson], enabled: Bool) {
        let identifiers = people.map { birthdayIdentifier(for: $0.id) }
        center.removePendingNotificationRequests(withIdentifiers: identifiers)

        guard enabled else { return }

        people.forEach { scheduleBirthdayNotification(for: $0) }
    }
